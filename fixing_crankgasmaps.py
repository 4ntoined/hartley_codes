...
def make_gasmaps(pathToScanDirectory):
    ...
    for xx in range(xsize): #for each pixel in the x
        for yy in range(ysize): #take a pixel in the y
            ...
            spect = dat[:,pixely,pixelx] #spectrum of the pixel
            wavex = waves[:,pixely,pixelx] #wavelengths
            emiss = findEmissions(wavex) #using wavelengths to find where emission bands are
            h2os,h2ol = emiss[0]
            co2s,co2l = emiss[1]
    ...
    fitter.writeto(pathToScanDirectory + "/cube_gasmaps_vtesting.fit")
    return
h1 = 2.59
h2 = 2.77
c1 = 4.17
...
a=[]
q=0
patho = '/chiron4/antojr/calibrated_ir/310.4007300'
make_gasmaps(patho)
quit()
